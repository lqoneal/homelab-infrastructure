#!/usr/bin/env bash
# Gate identity: OA-05
# Capability statement: Production execution-agent registry
...
# Required commands: zeus agent registry, zeus agent status
...
# Positive path: Through the authoritative CLI, demonstrate production execution-agent registry and capture the resulting production-observable state.
# Negative path: Present a malformed, unauthorized, stale, mismatched, or incomplete OA-05 request and verify Zeus rejects it without advancing state.
# Idempotency: Repeat the OA-05 observation or authorized request with the same identity and verify no duplicate state, event, evidence, or action.
...
# Regression scope: OA-01, OA-02, OA-03, OA-04
# Evidence requirements: repository identity and exact HEAD; command stdout, stderr, and return code; OA-05 positive and negative assertions; evidence integrity manifest and completion marker
...
exec "${SCRIPT_DIR}/../bin/pmct" run "OA-05" "$@"
