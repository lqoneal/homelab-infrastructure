#!/usr/bin/env bash
# Gate identity: OA-08
# Capability statement: Admission-driven dispatch authorization
...
# Required commands: zeus admission evaluate, zeus dispatcher probe
...
# Positive path: Through the authoritative CLI, demonstrate admission-driven dispatch authorization and capture the resulting production-observable state.
# Negative path: Present a malformed, unauthorized, stale, mismatched, or incomplete OA-08 request and verify Zeus rejects it without advancing state.
# Idempotency: Repeat the OA-08 observation or authorized request with the same identity and verify no duplicate state, event, evidence, or action.
...
# Regression scope: OA-01, OA-02, OA-03, OA-04, OA-05, OA-06, OA-07
# Evidence requirements: repository identity and exact HEAD; command stdout, stderr, and return code; OA-08 positive and negative assertions; evidence integrity manifest and completion marker
...
exec "${SCRIPT_DIR}/../bin/pmct" run "OA-08" "$@"
