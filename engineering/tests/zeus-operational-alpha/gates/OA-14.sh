#!/usr/bin/env bash
# Gate identity: OA-14
# Capability statement: Authority restoration coordination
...
# Required commands: zeus authority restoration
...
# Positive path: Through the authoritative CLI, demonstrate authority restoration coordination and capture the resulting production-observable state.
# Negative path: Present a malformed, unauthorized, stale, mismatched, or incomplete OA-14 request and verify Zeus rejects it without advancing state.
# Idempotency: Repeat the OA-14 observation or authorized request with the same identity and verify no duplicate state, event, evidence, or action.
...
# Regression scope: OA-01, OA-02, OA-03, OA-04, OA-05, OA-06, OA-07, OA-08, OA-09, OA-10, OA-11, OA-12, OA-13
# Evidence requirements: repository identity and exact HEAD; command stdout, stderr, and return code; OA-14 positive and negative assertions; evidence integrity manifest and completion marker
...
exec "${SCRIPT_DIR}/../bin/pmct" run "OA-14" "$@"
