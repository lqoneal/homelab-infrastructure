#!/usr/bin/env bash
# Gate identity: OA-06
# Capability statement: Production execution-agent qualification
...
# Positive path: Through the authoritative CLI, demonstrate production execution-agent qualification and capture the resulting production-observable state.
# Negative path: Present a malformed, unauthorized, stale, mismatched, or incomplete OA-06 request and verify Zeus rejects it without advancing state.
# Idempotency: Repeat the OA-06 observation or authorized request with the same identity and verify no duplicate state, event, evidence, or action.
...
# Regression scope: OA-01, OA-02, OA-03, OA-04, OA-05
# Evidence requirements: repository identity and exact HEAD; command stdout, stderr, and return code; OA-06 positive and negative assertions; evidence integrity manifest and completion marker
...
exec "${SCRIPT_DIR}/../bin/pmct" run "OA-06" "$@"
