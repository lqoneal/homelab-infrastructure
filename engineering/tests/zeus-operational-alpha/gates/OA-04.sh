#!/usr/bin/env bash
# Gate identity: OA-04
# Capability statement: Dispatcher activation
...
# Required commands: zeus dispatcher activation, zeus dispatcher status
...
# Positive path: Through the authoritative CLI, demonstrate dispatcher activation and capture the resulting production-observable state.
# Negative path: Present a malformed, unauthorized, stale, mismatched, or incomplete OA-04 request and verify Zeus rejects it without advancing state.
# Idempotency: Repeat the OA-04 observation or authorized request with the same identity and verify no duplicate state, event, evidence, or action.
...
# Regression scope: OA-01, OA-02, OA-03
# Evidence requirements: repository identity and exact HEAD; command stdout, stderr, and return code; OA-04 positive and negative assertions; evidence integrity manifest and completion marker
...
exec "${SCRIPT_DIR}/../bin/pmct" run "OA-04" "$@"
