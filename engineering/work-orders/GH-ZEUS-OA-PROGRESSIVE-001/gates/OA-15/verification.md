# OA-15 Operator Verification Guide

## Intent and implementation

This gate is intended to prove Zeus dispatches the admitted WOP to the selected qualified agent exactly once.
...
The package manifest and admission receipt must validate; `OA-15` must be the sole
...
1. Run `zeus gate show OA-15`. Expect JSON with `gate_id` equal to `OA-15` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-15`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-15`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-15`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-15/VERIFIED`.
...
7. Run `zeus explain OA-15` and inspect every file returned by
   `zeus gate evidence OA-15`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-15 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-15 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-15`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-16` without a new mission authorization. After OA-30,
