# OA-19 Operator Verification Guide

## Intent and implementation

This gate is intended to prove append-only capture of commands, outputs, state, timestamps, identities, checksums, and completion markers.
...
The package manifest and admission receipt must validate; `OA-19` must be the sole
...
1. Run `zeus gate show OA-19`. Expect JSON with `gate_id` equal to `OA-19` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-19`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-19`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-19`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-19/VERIFIED`.
...
7. Run `zeus explain OA-19` and inspect every file returned by
   `zeus gate evidence OA-19`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-19 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-19 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-19`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-20` without a new mission authorization. After OA-30,
