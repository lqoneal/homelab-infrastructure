# OA-23 Operator Verification Guide

## Intent and implementation

This gate is intended to prove durable pause behavior without inferred completion or duplicated effects.
...
The package manifest and admission receipt must validate; `OA-23` must be the sole
...
1. Run `zeus gate show OA-23`. Expect JSON with `gate_id` equal to `OA-23` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-23`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-23`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-23`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-23/VERIFIED`.
...
7. Run `zeus explain OA-23` and inspect every file returned by
   `zeus gate evidence OA-23`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-23 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-23 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-23`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-24` without a new mission authorization. After OA-30,
