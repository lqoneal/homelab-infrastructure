# OA-08 Operator Verification Guide

## Intent and implementation

This gate is intended to prove deterministic resolution of the selected mission to one immutable WOP.
...
The package manifest and admission receipt must validate; `OA-08` must be the sole
...
1. Run `zeus gate show OA-08`. Expect JSON with `gate_id` equal to `OA-08` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-08`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-08`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-08`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-08/VERIFIED`.
...
7. Run `zeus explain OA-08` and inspect every file returned by
   `zeus gate evidence OA-08`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-08 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-08 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-08`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-09` without a new mission authorization. After OA-30,
