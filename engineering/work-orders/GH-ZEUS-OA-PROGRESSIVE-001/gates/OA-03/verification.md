# OA-03 Operator Verification Guide

## Intent and implementation

This gate is intended to prove deterministic discovery of exactly one applicable Mission Contract.
...
The package manifest and admission receipt must validate; `OA-03` must be the sole
...
1. Run `zeus gate show OA-03`. Expect JSON with `gate_id` equal to `OA-03` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-03`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-03`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-03`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-03/VERIFIED`.
...
7. Run `zeus explain OA-03` and inspect every file returned by
   `zeus gate evidence OA-03`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-03 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-03 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-03`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-04` without a new mission authorization. After OA-30,
