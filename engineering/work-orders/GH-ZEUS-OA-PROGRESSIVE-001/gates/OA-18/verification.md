# OA-18 Operator Verification Guide

## Intent and implementation

This gate is intended to prove protected actions pause for valid operator approval and cannot bypass the approval boundary.
...
The package manifest and admission receipt must validate; `OA-18` must be the sole
...
1. Run `zeus gate show OA-18`. Expect JSON with `gate_id` equal to `OA-18` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-18`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-18`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-18`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-18/VERIFIED`.
...
7. Run `zeus explain OA-18` and inspect every file returned by
   `zeus gate evidence OA-18`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-18 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-18 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-18`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-19` without a new mission authorization. After OA-30,
