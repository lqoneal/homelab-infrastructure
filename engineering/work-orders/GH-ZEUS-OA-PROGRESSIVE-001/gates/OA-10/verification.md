# OA-10 Operator Verification Guide

## Intent and implementation

This gate is intended to prove bounded execution context, principal identity, authority lease, expiry, and revocation behavior.
...
The package manifest and admission receipt must validate; `OA-10` must be the sole
...
1. Run `zeus gate show OA-10`. Expect JSON with `gate_id` equal to `OA-10` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-10`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-10`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-10`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-10/VERIFIED`.
...
7. Run `zeus explain OA-10` and inspect every file returned by
   `zeus gate evidence OA-10`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-10 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-10 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-10`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-11` without a new mission authorization. After OA-30,
