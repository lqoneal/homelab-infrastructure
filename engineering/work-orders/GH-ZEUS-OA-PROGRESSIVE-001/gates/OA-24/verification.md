# OA-24 Operator Verification Guide

## Intent and implementation

This gate is intended to prove reconstruction from durable state and continuation from the first incomplete operation.
...
The package manifest and admission receipt must validate; `OA-24` must be the sole
...
1. Run `zeus gate show OA-24`. Expect JSON with `gate_id` equal to `OA-24` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-24`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-24`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-24`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-24/VERIFIED`.
...
7. Run `zeus explain OA-24` and inspect every file returned by
   `zeus gate evidence OA-24`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-24 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-24 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-24`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-25` without a new mission authorization. After OA-30,
