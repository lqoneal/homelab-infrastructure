# OA-07 Operator Verification Guide

## Intent and implementation

This gate is intended to prove Zeus selects only an eligible staged mission according to controlled priority and policy.
...
The package manifest and admission receipt must validate; `OA-07` must be the sole
...
1. Run `zeus gate show OA-07`. Expect JSON with `gate_id` equal to `OA-07` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-07`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-07`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-07`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-07/VERIFIED`.
...
7. Run `zeus explain OA-07` and inspect every file returned by
   `zeus gate evidence OA-07`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-07 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-07 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-07`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-08` without a new mission authorization. After OA-30,
