# OA-11 Operator Verification Guide

## Intent and implementation

This gate is intended to prove integrity-bound qualification and repository-preserving registration of execution agents.
...
The package manifest and admission receipt must validate; `OA-11` must be the sole
...
1. Run `zeus gate show OA-11`. Expect JSON with `gate_id` equal to `OA-11` and the
   complete contract. Any missing field is FAIL.
2. Run `zeus gate objective OA-11`. Expect the objective in this guide. A different
   objective is FAIL.
3. Run `zeus gate evidence OA-11`. Expect the evidence template and runtime directory.
4. Run `zeus verify OA-11`. Expect all positive, negative, replay, recovery, and
   cumulative checks to report PASS and create `runtime/evidence/OA-11/VERIFIED`.
...
7. Run `zeus explain OA-11` and inspect every file returned by
   `zeus gate evidence OA-11`. Confirm checksums, identities, timestamps, exit codes,
...
Reject with `zeus decline OA-11 --operator OPERATOR`; this persists a rejection and
stops fail closed. Accept only after PASS with
`zeus approve OA-11 --operator OPERATOR`. Confirm the receipt using
`zeus gate receipt OA-11`. Run `zeus resume`; the controller recognizes the receipt
and directly enables `OA-12` without a new mission authorization. After OA-30,
