...
def operator_text(value: Any, title: str | None = None) -> str:
    ...
    if value.get("operation") == "BETA" and isinstance(value.get("missions"), list):
        lines.extend([
            ...
            f"Recommended mission           : {value.get('recommended_mission') or 'NONE'}",
            f"Next authorized action        : {value.get('next_authorized_action') or 'NONE'}",
    ...
    for key in ("result", "mission_id", "classification", "lifecycle", "current_executable_mission", "recommended_mission",
                "next_authorized_action", "roadmap_id", "roadmap_revision", "mission_knowledge_revision", "status"):
