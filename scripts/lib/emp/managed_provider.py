...
def execute(*, repository: Path | str, prompt: str, authorized_paths: Iterable[str], execution_id: str | None = None,
    ...
    try: process = subprocess.Popen(command, cwd=root, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, start_new_session=True)
    ...
    record = {"operation_id":"OPERATION-BETA", "emm_id":"OPERATION-BETA-EMM", "wop_id":None, "gate_or_transaction_id":None,
              "zeus_execution_id":execution, "provider":"CODEX", "provider_session_id":None, "start_time":started_at, "end_time":ended_at,
