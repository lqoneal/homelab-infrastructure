"""Integrity-bound production verification for Progressive OA-04."""
...
from scripts.lib.emp.oa04_implementation import validate_evidence


class OA04GateVerificationError(ValueError):
    """OA-04 verification cannot safely be represented as current."""


def _directory(root: Path) -> Path:
    return root / progressive_oa.PACKAGE_PATH / "runtime/evidence/OA-04"
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise OA04GateVerificationError(f"{path}: {error}") from error
    if not isinstance(value, dict):
        raise OA04GateVerificationError(f"{path}: object required")
    return value
...
def _current(root: Path, boundary: str) -> dict[str, Any]:
    ...
    if state.get("active_gate") != "OA-04" or gate.get("state") != "AWAITING_OPERATOR_VERIFICATION":
        raise OA04GateVerificationError("OA-04 is not awaiting operator verification")
    if gate.get("acceptance_receipt") is not None:
        raise OA04GateVerificationError("OA-04 acceptance already exists")
    for gate_id, item in state["gates"].items():
        if gate_id > "OA-04" and (
            item.get("state") != "PENDING" or item.get("acceptance_receipt") is not None
        ):
            raise OA04GateVerificationError(f"unexpected later-gate activity: {gate_id}")
    ...
    if context["governing_authority"]["digest"] != authority["authority_digest"]:
        raise OA04GateVerificationError("context reconstruction authority mismatch")
    ...
    return {
        ...
        "implementation_evidence": str(
            (
                ...
                / "runtime/evidence/OA-04/IMPLEMENTATION.json"
            ).relative_to(root)
        ),
...
def _outside_evidence(value: dict[str, Any]) -> list[dict[str, str]]:
    prefix = str(progressive_oa.PACKAGE_PATH / "runtime/evidence/OA-04/")
...
def _marker(evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    context = inputs.get("project_operational_context")
    if not isinstance(context, dict):
        raise OA04GateVerificationError(
            "verification evidence predates Project and Operational Context Reconstruction"
        )
    resolution = context["mission_resolution"]
    value = {
        "schema_version": 1,
        ...
        "mission_id": resolution["mission"]["mission_id"],
        "wop_id": resolution["wop"]["wop_id"],
        "execution_definition_digest": resolution["execution_definition"]["digest"],
...
def validate_marker(root: Path | str) -> dict[str, Any]:
    ...
    if evidence.get("canonical_evidence_digest") != canonical_digest(
        ...
        raise OA04GateVerificationError("OA-04 marker integrity or binding mismatch")
    current = _current(repository, "oa04_marker_validation")
    ...
    if (
        ...
        raise OA04GateVerificationError("OA-04 verification is stale")
    return marker
...
def verify(root: Path | str) -> dict[str, Any]:
    ...
    if marker_path.is_file():
        ...
        except OA04GateVerificationError:
            ...
            for source in (evidence_path, marker_path):
                ...
                if destination.exists():
                    if destination.read_bytes() != source.read_bytes():
                        raise OA04GateVerificationError("verification archive collision")
                    source.unlink()
                else:
                    os.replace(source, destination)

    authoritative_inputs = _current(repository, "oa04_verification_start")
    ...
    for name, arguments in commands.items():
        if os.environ.get("ZEUS_OA04_VERIFY_INTERRUPT") == name:
            raise OA04GateVerificationError(f"interrupted during verification: {name}")
        checks[name] = run(repository, arguments)
        if checks[name]["exit_code"]:
            raise OA04GateVerificationError(f"OA-04 verification failed: {name}")

    before_evidence = _current(repository, "oa04_verification_evidence")
    if _stable(before_evidence) != _stable(authoritative_inputs):
        raise OA04GateVerificationError("Mission Resolution changed during verification")
    evidence = {
        "schema_version": 1,
        "gate_id": "OA-04",
        "package_id": progressive_oa.PACKAGE,
    ...
    if os.environ.get("ZEUS_OA04_VERIFY_INTERRUPT_AFTER_EVIDENCE") == "1":
        raise OA04GateVerificationError("interrupted after verification evidence")
    current = _current(repository, "oa04_verification_marker")
    if _stable(current) != _stable(before_evidence):
        raise OA04GateVerificationError("Mission Resolution changed before marker")
