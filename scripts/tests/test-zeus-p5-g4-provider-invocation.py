...
class ProviderInvocationFoundationTests(unittest.TestCase):
    ...
    def test_publication_advance_preserves_invocation_provenance(self):
        ...
        self.assertEqual(value["current_published_baseline"], current)
    ...
    def test_mission_projection_agrees(self):
        ...
        self.assertTrue(status["provider_acknowledged"])
        self.assertEqual(status["next_authorized_action"], "OPERATOR_REVIEW_LEGACY_LIFECYCLE_RECONCILIATION")
        self.assertEqual(status["invocation_provenance_baseline"], "b37a5fb2e11df8026afeff1bd231902cd54711ac")
        current = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT,
                                 capture_output=True, text=True, check=True).stdout.strip()
        self.assertEqual(status["current_published_baseline"], current)
        ...
        self.assertEqual(next_value["next_authorized_action"], "OPERATOR_REVIEW_LEGACY_LIFECYCLE_RECONCILIATION")
