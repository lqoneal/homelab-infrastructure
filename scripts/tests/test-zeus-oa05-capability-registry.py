...
class OA05CapabilityRegistryTests(unittest.TestCase):
 def test_registry_is_complete(self):
  ...
  self.assertEqual(5, len(capability_registry.list_capabilities(ROOT)["capabilities"]))
 ...
 def test_current_lifecycle_is_bound(self):
  ...
  self.assertEqual("ACTIVE",wop["status"]); self.assertEqual("OA-06",resolve(ROOT)["active_gate"])
